...
def student_t(x, mean, scale, deg_free):
    const = tf.lgamma(tf.cast((deg_free + 1.) * 0.5, _settings.tf_float))\
        - tf.lgamma(tf.cast(deg_free * 0.5, _settings.tf_float))\
        - 0.5*(tf.log(tf.square(scale)) + tf.cast(tf.log(deg_free), _settings.tf_float)
               + np.log(np.pi))
    const = tf.cast(const, _settings.tf_float)
...
def multivariate_normal(x, mu, L):
    ...
    num_col = tf.cast(num_col, _settings.tf_float)
    num_dims = tf.cast(tf.shape(x)[0], _settings.tf_float)
