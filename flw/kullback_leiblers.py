...
def gauss_kl_white(q_mu, q_sqrt):
    ...
    KL += -0.5 * tf.cast(tf.reduce_prod(tf.shape(q_sqrt)[1:]), _settings.tf_float)  # constant term
...
def gauss_kl_white_diag(q_mu, q_sqrt):
    ...
    KL = 0.5 * tf.reduce_sum(tf.square(q_mu))  # Mahalanobis term
    KL += -0.5 * tf.cast(tf.size(q_sqrt), _settings.tf_float)
...
def gauss_kl_diag(q_mu, q_sqrt, K):
    ...
    num_latent = tf.cast(tf.shape(q_sqrt)[1], _settings.tf_float)
    KL += num_latent * 0.5 * tf.reduce_sum(
        tf.log(tf.square(tf.matrix_diag_part(L))))  # Prior log-det term.
    KL += -0.5 * tf.cast(tf.size(q_sqrt), _settings.tf_float)  # constant term
    KL += -0.5 * tf.reduce_sum(tf.log(tf.square(q_sqrt)))  # Log-det of q-cov
    L_inv = tf.matrix_triangular_solve(L, tf.eye(tf.shape(L)[0], dtype=_settings.tf_float), lower=True)
...
def gauss_kl(q_mu, q_sqrt, K):
    ...
    alpha = tf.matrix_triangular_solve(L, q_mu, lower=True)
    KL = 0.5 * tf.reduce_sum(tf.square(alpha))  # Mahalanobis term.
    num_latent = tf.cast(tf.shape(q_sqrt)[2], _settings.tf_float)
