...
class Kern():
    ...
    def exKxz(self, Z, Xmu, Xcov):
        ...
        with tf.control_dependencies([
            tf.assert_equal(tf.shape(Xmu)[1], tf.constant(D, dtype=_settings.tf_int),
    ...
    def _check_quadrature(self):
        if _settings.numerics.ekern_quadrature == "warn":
            warnings.warn("Using numerical quadrature for kernel expectation of %s. Use gpflow.ekernels instead." %
                          str(type(self)))
        if _settings.numerics.ekern_quadrature == "error" or self.num_gauss_hermite_points == 0:
            raise RuntimeError("_settings indicate that quadrature may not be used.")
    ...
    def _slice(self, X, X2):
        ...
        with tf.control_dependencies([
                            tf.assert_equal(tf.shape(X)[1],
                                            tf.constant(self.input_dim, dtype=_settings.tf_int))]):
            X = tf.identity(X)
...
class Static(Kern):
    ...
    def __init__(self, input_dim, variance=1.0, active_dims=None, name=None):
        ...
        self.variance_temp = tf.Variable(variance, dtype=_settings.tf_float)
...
class White(Static):
    ...
    def K(self, X, X2=None, presliced=False):
        if X2 is None:
            ...
            return tf.zeros(shape, _settings.tf_float)
...
class Stationary(Kern):
    ...
    def __init__(self, input_dim, variance=1.0, lengthscales=None,
        ...
        self.variance = tf.Variable(np.square(variance))
        if ARD:
            if lengthscales is None:
                lengthscales = np.ones(input_dim, _settings.np_float)
            else:
                # accepts float or array:
                lengthscales = lengthscales * np.ones(input_dim, _settings.np_float)
...
class ArcCosine(Kern):
    ...
    def __init__(self, input_dim,
        ...
        if ARD:
            if weight_variances is None:
                weight_variances = np.ones(input_dim, _settings.np_float)
            else:
                # accepts float or array:
                weight_variances = weight_variances * np.ones(input_dim, _settings.np_float)
    ...
    def Kdiag(self, X, presliced=False):
        ...
        theta = tf.constant(0., _settings.tf_float)
