...
class Transform(): # pylint: disable=W0223
    pass
...
class Identity(Transform):
    ...
    def log_jacobian_tensor(self, x):
        return tf.zeros((1,), _settings.tf_float)
...
class Log1pe(Transform):
    ...
    def backward(self, y):
        ...
        """
        ys = np.maximum(y - self._lower, np.finfo(_settings.np_float).eps)
...
class Rescale(Transform):
    ...
    def log_jacobian_tensor(self, x):
        return tf.cast(tf.reduce_prod(tf.shape(x)), _settings.tf_float) * \
...
class DiagMatrix(Transform):
    ...
    def log_jacobian_tensor(self, x):
        return tf.zeros((1,), _settings.tf_float) + self._positive_transform.log_jacobian_tensor(x)
...
class LowerTriangular(Transform):
    ...
    def forward(self, x):
        ...
        var = np.zeros((matsize, matsize, self.num_matrices), _settings.np_float)
